...
class Gemma3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Skip for sliding attention layers - they use local attention and don't need prefix biases
        modified_attention_mask = attention_mask
        if isinstance(past_key_values, CompactedPrefixCache) and not self.is_sliding:
        ...
        attn_output, attn_weights = attention_interface(
            ...
            key_states,
            value_states,
            modified_attention_mask, # last dimension should only be 2242 not 6825. (1219+1023. 1219 is length of key states.)
...
class Gemma3TextModel(Gemma3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # For CompactedPrefixCache, we need to offset position_ids for RoPE
        # but NOT for mask creation (mask needs actual cache positions)
        ...
        if isinstance(past_key_values, CompactedPrefixCache):
            position_ids = cache_position.unsqueeze(0)
            rope_base = past_key_values.rope_base()
        ...
        if not isinstance(causal_mask_mapping := attention_mask, dict):
            ...
            # For CompactedPrefixCache, the sliding layer still needs to use the original seq len for masking instead of compacted sequence length.
            if isinstance(past_key_values, CompactedPrefixCache):
                sliding_mask_kwargs["cache_position"] = cache_position + rope_base
                if position_ids is not None:
                    sliding_mask_kwargs["position_ids"] = position_ids + rope_base
                if attention_mask is not None:
                    # Prepend rope_base 1s to cover the original sequence positions
                    prefix_ones = torch.ones(
                        (attention_mask.shape[0], rope_base),
                        device=attention_mask.device,
                        dtype=attention_mask.dtype,
                    )
                    sliding_mask_kwargs["attention_mask"] = torch.cat([prefix_ones, attention_mask], dim=1)
            ...
            if isinstance(past_key_values, CompactedPrefixCache):
                mask = causal_mask_mapping['full_attention']
        ...
        # Both global and local RoPE use the same position_ids - they differ only in rope_theta
        rope_pos_ids = rope_position_ids if rope_position_ids is not None else position_ids
        position_embeddings_global = self.rotary_emb(hidden_states, rope_pos_ids)
        position_embeddings_local = self.rotary_emb_local(hidden_states, rope_pos_ids)
